...
class ClienteRepository {
  ...
  Future<ClienteFicha> obtenerFicha(String id) async {
    final row = await _db
        .from('clientes')
        .select(
  ...
  Future<List<CuotaCuentaCorriente>> obtenerCuotas(String clienteId) async {
  ...
  Future<ResumenCliente> obtenerResumen(String clienteId) async {
  ...
  Future<String> insertar(ClienteFicha ficha) async {
    final row = await _db
        .from('clientes')
  ...
  Future<String?> actualizar(ClienteFicha ficha) async {
  ...
  Future<List<TipoDocIdentidadOption>> obtenerTiposDocIdentidad() async {
    final rows = await _db
  ...
  Future<List<ZonaOption>> obtenerZonas() async {
    final rows = await _db
        .from('zonas')
        .select('id, nombre')
        .eq('activo', true)
  ...
  Future<List<ClienteListado>> listar(String termino) async {
